...
function DynamicForm(parent, coreConfig, formOptions) {
  ...
  if (formOptions === undefined || formOptions.inputs === undefined) {
    throw new Error("Cannot create a DynamicForm without formOptions");
  }
  if (parent === undefined) {
    throw new Error("Cannot create a DynamicForm without a structure to bind to");
  }

  let addIcon = coreConfig.add_icon || b64AddIcon;
  let removeIcon = coreConfig.remove_icon || b64RemoveIcon;

  // FIXME: add a way to use nested divs
  var tbody = parent.getElementsByTagName('tbody')[0];
  let inputOpts = formOptions.inputs;
  ...
  var addInputs = function(tableRow) {
    ...
    for (var config of inputOpts) {
      ...
      input.className = config.className || coreConfig.defaultClass;
  ...
  var organizeForm = function() {
    ...
    for (var r = 0, length = rows.length; r < length; r++) {
